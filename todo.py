...
class Combiner(nn.Module):
    ...
    def forward(self, image_features, text_features):
        ...
        # Pass through the fully connected layer to generate fused features
...
class Model(nn.Module):
    ...
    def encode_image(self, image_paths):
        ...
        """
        images=[self.preprocess(Image.open(i_path).convert("RGB")) for i_path in image_paths]

        image_input = torch.tensor(np.stack(images)).to(device)
     
        vision_features=self.model.encode_image(image_input)
